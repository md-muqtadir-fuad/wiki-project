...
বিশটি
    ...
    function collectApiCandidateTitlesFromSegments(segments) {
        ...
        var candidates = Array.from(candidateSet).sort(function (a, b) {
            ...
            return (wordPriority[aw] || 99) - (wordPriority[bw] || 99) || a.localeCompare(b);
        });
    ...
    function queryExistingTitlesWithMediaWikiApi(candidateTitles) {
        ...
        function runNextBatch() {
            ...
            return api.get({
                ...
                debugLog(
                    "[bn-internal-linker] API batch result:",
                    "pages =", pages.length,
                    "existing total =", existing.size
                );
            }).then(runNextBatch);
        }
    ...
    function buildTitleDataFromExistingTitles(existingTitles) {
        ...
        if (data.set.size === 0) {
            debugWarn("[bn-internal-linker] API worked, but no valid existing Bengali titles matched the generated candidates.");
            return data;
        }
    ...
    function loadTitleDataForSegments(segments) {
        if (!CONFIG.useMediaWikiApiTitleLookup) {
            return Promise.reject(
                new Error("Full title-list loading is disabled in this version. Keep useMediaWikiApiTitleLookup: true.")
            );
        }
