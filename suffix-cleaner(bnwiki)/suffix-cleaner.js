...
/*
 * suffix-cleaner.js
 * বাংলা হাইফেনযুক্ত প্রত্যয় পরিষ্কারক for bn.wikipedia.org source editor.
 *
 * Safely handles:
 *   1) Bengali words:  বাংলা-এর → বাংলার, দেশ-এর → দেশের, বই-তে → বইতে
 *   2) Exceptions:     চ্যাং-এর → চ্যাংয়ের, এআই-এর → এআইয়ের
 *   3) Numbers:        ১৯৩২-এর → ১৯৩২ এর, ''২''-এর → ২ এর
 *   4) Wiki links:     [[আমেরিকান এক্সপেরিয়েন্স]]-এর
...
(function () {
    ...
    mw.loader.using(['mediawiki.util']).then(function () {
        var CONFIG = {
            editSummaryNote: 'প্রত্যয় পরিষ্কারক: বাংলা হাইফেনযুক্ত প্রত্যয় সংশোধন',
        ...
        var BN_WORD = '[\\u0980-\\u09FF]+';
        var DIGIT = '[0-9০-৯]+';
        var SUFFIX_RE_PART = '(এর|র|কে|তে|এ)';
         ...
         *   চ্যাং-এ → চ্যাংয়ে
         */
        var ER_YE_EXCEPTION_ENDINGS = 'ংঙইঈঅআউঊঋএঐওঔ';
        var ER_ONLY_R_ENDINGS = 'ািীুূৃেৈোৌ';

        var E_YE_EXCEPTION_ENDINGS = 'ংঙইঈঅআউঊঋএঐওঔ';
        var E_TO_TE_ENDINGS = 'িী';
        var E_TO_Y_ENDINGS = 'া';
        ...
        function appendErSuffix(base) {
            ...
            if (endsWithAny(base, ER_YE_EXCEPTION_ENDINGS)) {
                return base + 'য়ের';
            }

            if (endsWithAny(base, ER_ONLY_R_ENDINGS)) {
                return base + 'র';
            }
        ...
        function appendESuffix(base) {
            if (!base) {
                return base;
            }

            if (endsWithAny(base, E_YE_EXCEPTION_ENDINGS)) {
                return base + 'য়ে';
            }

            if (endsWithAny(base, E_TO_TE_ENDINGS)) {
                return base + 'তে';
            }

            if (endsWithAny(base, E_TO_Y_ENDINGS)) {
                return base + 'য়';
            }
        ...
        function cleanWikiLinkSuffixes(text) {
            var linkSuffixRegex = new RegExp(
                '\\[\\[([^\\]\\n]+?)\\]\\]([\'"”“’‘]{0,5})\\s*-\\s*' + SUFFIX_RE_PART,
                'g'
            );

            return text.replace(linkSuffixRegex, function (match, content, closingMarks, suffix) {
                var parsed = splitWikiLinkContent(content);
                ...
                var preservedClosingMarks = closingMarks || '';

                if (parsed.hasPipe) {
                    return '[[' + parsed.target + '|' + newDisplayText + ']]' + preservedClosingMarks;
                }

                if (newDisplayText === parsed.target) {
                    return '[[' + parsed.target + ']]' + preservedClosingMarks;
                }
        ...
        function openSourceEditMode() {
            ...
            if (!pageName) {
                notify('প্রত্যয় পরিষ্কারক শুধু উৎস সম্পাদনা মোডে কাজ করে। অনুগ্রহ করে উৎস সম্পাদনা মোড খুলুন।', 'warn');
                return;
            }

            var ok = window.confirm(
                'প্রত্যয় পরিষ্কারক শুধু উৎস সম্পাদনা মোডে কাজ করে।\n\nএখন উৎস সম্পাদনা মোডে খুলবেন?'
            );

            if (!ok) {
                notify('অনুগ্রহ করে উৎস সম্পাদনা মোডে গিয়ে প্রত্যয় পরিষ্কারক চালান।', 'warn');
                return;
            }
        ...
        function runCleaner() {
            ...
            if (!hasSelection && CONFIG.askBeforeFullPageClean) {
                var ok = window.confirm('কোনো টেক্সট নির্বাচন করা হয়নি। পুরো সম্পাদনায় প্রত্যয় পরিষ্কারক চালাবেন?');
                if (!ok) {
                    return;
                }
            }
        ...
        function addSidebarButton() {
            ...
            var link = mw.util.addPortletLink(
                ...
                'উৎস সম্পাদনা মোডে বাংলা হাইফেনযুক্ত প্রত্যয় পরিষ্কার করে'
            );

            link = document.getElementById(CONFIG.portletLinkId) || link;
        ...
        var sidebarRetryCount = 0;

        function addSidebarButtonWithRetry() {
            if (document.getElementById(CONFIG.portletLinkId)) {
                return;
            }
            ...
            if (sidebarRetryCount < 20) {
                sidebarRetryCount++;
                window.setTimeout(addSidebarButtonWithRetry, 250);
            }
        }
        ...
        function handleRecentChangesFilters() {
            ...
            mw.util.addPortletLink(
                ...
                'ছাঁকা সাম্প্রতিক পরিবর্তন',
                CONFIG.recentChangesLinkId,
                'বট, বিষয়শ্রেণীকরণ, উইকিউপাত্ত ও উইকিফাংশনস লুকিয়ে সাম্প্রতিক পরিবর্তন খুলুন'
            );
        }
